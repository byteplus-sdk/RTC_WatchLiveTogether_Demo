...
@property (nonatomic, strong) BaseIMComponent *imCompoments;
    ...
    switch (type) {
        ...
        case LiveShareButtonTypeBeauty: {
            if (self.beautyCompoments) {
                [self.beautyCompoments showWithView:self.view dismissBlock:^(BOOL result) {
...
- (void)handelPortraitUI {
    ...
    [self.imCompoments updaetHidden:NO];
...
- (void)handleLandscapeUI {
    ...
    [self.imCompoments updaetHidden:YES];
...
- (void)requestChangeLiveURLToServer {
    [LiveShareRTSManager requestChangeVideo:[LiveShareDataManager shared].roomModel.roomID
                             ...
                             videoDirection:[LiveShareDataManager shared].roomModel.videoDirection
                                      block:^(LiveShareRoomModel * _Nonnull roomModel, RTSACKModel * _Nonnull model) {
        if (!model.result) {
...
- (void)requestStartShareToServer {
    ...
    [LiveShareRTSManager requestJoinWatch:[LiveShareDataManager shared].roomModel.roomID
        ...
        if (!model.result) {
            ...
            if (model.code == 649) {
                message = LocalizedString(@"url_only_use_once_a_day");
            }
            ...
            [[AlertActionManager shareAlertActionManager] showWithMessage:message actions:@[alertModel]];
        }
        else {
...
- (void)handleVideoLoadFailure {
    ...
    [[AlertActionManager shareAlertActionManager] showWithMessage:LocalizedString(@"failed_parse_url") actions:@[alertModel]];
    
}
- (void)handleVideoPlayComplete {
    [[ToastComponent shareToastComponent] showWithMessage:LocalizedString(@"live_ended_please_enter_new")];
}
...
- (LiveShareNavView *)navView {
    if (!_navView) {
        ...
        NSString *rooId = [LiveShareDataManager shared].roomModel.roomID;
        _navView.title = [NSString stringWithFormat:LocalizedString(@"live_room_id_:%@"),rooId];
        __weak typeof(self) weakSelf = self;
...
- (BaseIMComponent *)imCompoments {
    if (!_imCompoments) {
        _imCompoments = [[BaseIMComponent alloc] initWithSuperView:self.view];
    }
...
- (UIView *)fullScreenView {
    if (!_fullScreenView) {
        ...
        label.text = LocalizedString(@"active_full_screen_mode");
