...
- (void)viewWillAppear:(BOOL)animated {
    ...
    // Enable/Disable audio streaming according to the preview page configuration.
    // Because the frequent opening/closing of microphone acquisition affects the call effect, the control audio streaming solution is adopted.
    [[LiveShareRTCManager shareRtc] publishAudioStream:[LiveShareMediaModel shared].enableAudio];
    
    // Enable/Disable camera acquisition according to the preview page configuration
...
- (void)onUserJoined:(LiveShareUserModel *)userModel {
    // Block self from entering the room. Entering the room by yourself has been added in the room entering interface
...
- (void)onReceivedUserMessage:(LiveShareUserModel *)userModel message:(NSString *)message {
    // Block messages sent by yourself, which have been added locally when sending.
    if ([userModel.uid isEqualToString:[LocalUserComponent userModel].uid]) {
        return;
    }
