...
public class PreviewActivity extends SolutionBaseActivity {
    ...
    protected void initUi() {
        ...
        mViewBinding.joinRoomBtn.setOnClickListener(DebounceClickListener.create(v -> {
            ...
            if (TextUtils.isEmpty(input)) {
                SolutionToast.show(this, getString(R.string.room_id_field_must_not_blank), Toast.LENGTH_SHORT);
                return;
            }
        ...
        boolean requested = mRTSClient.joinRoom(roomId,
                ...
                new IRequestCallback<JoinRoomResponse>() {
                    ...
                    public void onSuccess(JoinRoomResponse data) {
                        ...
                        if (room == null) {
                            onError(RTSBaseClient.ERROR_CODE_DEFAULT, getString(R.string.failed_get_room_information_when_entering));
                        } else if (TextUtils.isEmpty(room.rtcToken)) {
                            onError(RTSBaseClient.ERROR_CODE_DEFAULT, getString(R.string.failed_generate_token_when_entering_room));
                        } else {
                    ...
                    public void onError(int errorCode, String message) {
                        ...
                        if (RTSBaseClient.ERROR_CODE_USERNAME_SAME == errorCode) {
                            SolutionToast.show(getString(R.string.user_already_in_room));
                        } else if (RTSBaseClient.ERROR_CODE_ROOM_FULL == errorCode) {
                            SolutionToast.show(getString(R.string.room_you_join_full));
                        } else {
    ...
    private void setLocalRenderView() {
        ...
        VideoCanvas videoCanvas = new VideoCanvas(renderView, VideoCanvas.RENDER_MODE_HIDDEN);
    ...
    private void setupInputStatus() {
        ...
        if (Pattern.matches(ROOM_INPUT_REGEX, mViewBinding.roomIdEt.getText().toString())) {
            if (mRoomIdOverflow) {
                ...
                mViewBinding.roomIdWaringTv.setText(R.string.no_more_than_characters);
            ...
            if (roomIDLength > 0) {
                mViewBinding.roomIdWaringTv.setVisibility(View.VISIBLE);
                mViewBinding.roomIdWaringTv.setText(R.string.please_enter_numbers_english_letters);
            } else {
                mViewBinding.roomIdWaringTv.setVisibility(View.INVISIBLE);
            }
        }
